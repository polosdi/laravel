...
<head>
    ...
    <title>Laporan PKL – SIMPKL</title>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-cancel {
            ...
            background: white; font-family: inherit; font-size: 14px; cursor: pointer;
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('wakasek.laporan') }}" class="sidebar-item active">
            <div class="sidebar-item-icon">📄</div>
            Laporan PKL
        </a>
...
<main class="main">
    <div class="topbar">
        <div>
            <div class="topbar-title">Laporan PKL</div>
            <div class="topbar-sub">Kelola persetujuan laporan PKL siswa</div>
        </div>
    ...
    <div class="content">
        <div class="page-header">
            <div>
                <div class="page-header-title">Laporan PKL</div>
                <div class="page-header-sub">Total {{ $daftarLaporan->count() }} laporan masuk</div>
            </div>
            <span class="badge-count">{{ $daftarLaporan->count() }} Laporan</span>
        </div>
        ...
        <div class="filter-tabs">
            <button class="filter-tab active" data-filter="semua">Semua ({{ $daftarLaporan->count() }})</button>
            <button class="filter-tab" data-filter="pending">⏳ Menunggu ({{ $daftarLaporan->where('status_wakasek','pending')->count() }})</button>
            <button class="filter-tab" data-filter="disetujui">✅ Disetujui ({{ $daftarLaporan->where('status_wakasek','disetujui')->count() }})</button>
        </div>
        ...
        <div class="search-bar" style="margin-bottom: 20px;">
            <div class="search-input-wrap">
                <span>🔍</span>
                <input type="text" class="search-input" id="searchInput" placeholder="Cari nama siswa atau judul laporan...">
            </div>
        </div>

        <!-- TABLE -->
        <div class="card">
            <table class="pengajuan-table" id="laporanTable">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Siswa</th>
                ...
                <tbody>
                    @forelse($daftarLaporan as $i => $laporan)
                    <tr data-status="{{ $laporan->status_wakasek }}">
                        <td style="color: var(--text-muted); font-size: 13px;">{{ $i + 1 }}</td>
                        <td>
                            <div class="siswa-wrap">
                                <div class="siswa-avatar">
                                    {{ strtoupper(substr($laporan->siswa->nama_depan ?? '?', 0, 1)) }}
                                </div>
                                <div>
                                    <div class="siswa-name">{{ $laporan->siswa->nama_depan ?? '-' }} {{ $laporan->siswa->nama_belakang ?? '' }}</div>
                                    <div class="siswa-sub">{{ $laporan->siswa->email ?? '-' }}</div>
                                </div>
                            </div>
                        </td>
                        <td>
                        ...
                        <td>
                            @if($laporan->jenis_laporan === 'mingguan')
                                <span class="badge" style="background: rgba(102,126,234,0.1); color: var(--primary);">📅 Mingguan</span>
                            @else
                                <span class="badge" style="background: rgba(139,92,246,0.1); color: #8b5cf6;">📝 Akhir</span>
                            @endif
                        </td>
                        <td>
                            @if($laporan->status_pembimbing === 'disetujui')
                                <span class="badge badge-disetujui">✅ Disetujui</span>
                            @elseif($laporan->status_pembimbing === 'revisi')
                                <span class="badge badge-pending">🔄 Revisi</span>
                            @else
                                <span class="badge badge-pending">⏳ Pending</span>
                            @endif
                        </td>
                        <td>
                            @if($laporan->status_wakasek === 'disetujui')
                                <span class="badge badge-disetujui">✅ Disetujui</span>
                            @else
                                <span class="badge badge-pending">⏳ Pending</span>
                            @endif
                        ...
                        <td>
                            @if($laporan->file_path)
                                <a href="{{ asset('storage/' . $laporan->file_path) }}" target="_blank"
                                   style="color: var(--primary); font-size: 13px; text-decoration: none; font-weight: 600;">
                                    📎 Lihat File
                                </a>
                            @else
                        ...
                        <td>
                            @if($laporan->status_wakasek === 'pending' && $laporan->status_pembimbing === 'disetujui')
                            <div class="action-wrap">
                                <button class="btn-approve"
                                    onclick="showModal('approve', {{ $laporan->id }}, '{{ addslashes($laporan->judul_laporan) }}')">
                                    ✅ Setujui
                                </button>
                            </div>
                            @else
                    ...
                    <tr>
                        <td colspan="8">
                            <div class="empty-state">
                                ...
                                <div class="empty-state-text">Belum ada laporan PKL masuk.</div>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <div class="modal">
        ...
        <form method="POST" id="modalForm">
            ...
            <div class="modal-actions">
                ...
                <button type="submit" class="btn-confirm-approve" id="modalBtn">Setujui</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function showModal(type, id, judul) {
        const overlay = document.getElementById('modalOverlay');
        const form = document.getElementById('modalForm');
        form.action = `/dashboard/wakasek/laporan/${id}/approve`;
        document.getElementById('modalTitle').textContent = '✅ Setujui Laporan';
        document.getElementById('modalSub').textContent = `Kamu akan menyetujui laporan "${judul}". Lanjutkan?`;
