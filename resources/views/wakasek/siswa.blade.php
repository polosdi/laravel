...
<head>
    ...
    <title>Daftar Siswa PKL – SIMPKL</title>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ─── KELAS GROUP ─── */
        .kelas-group { margin-bottom: 32px; }
        .kelas-header {
            display: flex;
            align-items: center;
            gap: 12px;
        ...
        .siswa-table tr:hover td { background: rgba(102,126,234,0.03); }
        ...
        .siswa-name-wrap { display: flex; align-items: center; gap: 12px; }
        .siswa-name { font-weight: 600; color: var(--text); }
        .siswa-nis { font-size: 12px; color: var(--text-muted); margin-top: 2px; }
        ...
        .badge-pkl { background: rgba(16,185,129,0.1); color: #10b981; }
        .badge-belum { background: rgba(107,114,128,0.1); color: #6b7280; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('wakasek.siswa') }}" class="sidebar-item active">
            <div class="sidebar-item-icon">👨‍🎓</div>
            Data Siswa
        </a>
...
<main class="main">
    ...
    <div class="topbar">
        <div>
            <div class="topbar-title">Daftar Siswa PKL</div>
            <div class="topbar-sub">Data seluruh siswa dikelompokkan per kelas</div>
        </div>
    ...
    <div class="content">
        <div class="page-header">
            <div>
                <div class="page-header-title">Data Siswa</div>
                <div class="page-header-sub">Total {{ $totalSiswa }} siswa terdaftar</div>
            </div>
        ...
        <div class="toolbar">
            <div class="search-input-wrap">
                <span>🔍</span>
                <input type="text" class="search-input" id="searchInput" placeholder="Cari nama atau NIS siswa...">
            </div>
            <div class="filter-tabs">
                <button class="filter-tab active" data-filter="semua">Semua</button>
                <button class="filter-tab" data-filter="pkl">✅ Sudah PKL</button>
                <button class="filter-tab" data-filter="pending">⏳ Menunggu</button>
                <button class="filter-tab" data-filter="belum">❌ Belum PKL</button>
            </div>
        </div>

        <!-- GROUPED BY KELAS -->
        @forelse($siswaPerKelas as $kelas => $siswas)
        <div class="kelas-group" data-kelas="{{ $kelas }}">
            <div class="kelas-header">
                <div class="kelas-title">📚 Kelas {{ $kelas }}</div>
                <div class="kelas-count">{{ $siswas->count() }} siswa</div>
            </div>
            <div class="card">
                <table class="siswa-table">
                    ...
                    <tbody>
                        ...
                        @endphp
                        <tr class="siswa-row" data-status="{{ $statusPkl }}">
                            <td style="color: var(--text-muted); font-size: 13px;">{{ $i + 1 }}</td>
                            <td>
                                <div class="siswa-name-wrap">
                                    ...
                                    <div>
                                        ...
                                        <div class="siswa-nis">NIS: {{ $siswa->profilSiswa->nis ?? '-' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>{{ $siswa->profilSiswa->jurusan ?? '-' }}</td>
                            <td>{{ $siswa->profilSiswa->no_hp ?? '-' }}</td>
                            <td>
                                @if($statusPkl === 'pkl')
                                    <span class="badge badge-pkl">✅ Sudah PKL</span>
                                @elseif($statusPkl === 'pending')
                                    <span class="badge badge-pending">⏳ Menunggu Persetujuan</span>
                                @elseif($statusPkl === 'ditolak')
                                    <span class="badge badge-ditolak">❌ Ditolak</span>
                                @else
                                    <span class="badge badge-belum">— Belum PKL</span>
                                @endif
                            </td>
                        </tr>
                        @endforeach
        ...
        <div class="card">
            <div class="empty-state">
                ...
                <div class="empty-state-text">Belum ada data siswa terdaftar.</div>
            </div>
        </div>
        @endforelse
    </div>
