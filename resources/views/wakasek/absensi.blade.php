...
<head>
    ...
    <title>Rekap Absensi – SIMPKL</title>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sidebar-section-label {
            ...
            color: var(--text-muted); padding: 0 8px; margin: 16px 0 8px;
        }
        .sidebar-item {
        ...
        .progress-fill { height: 100%; border-radius: 99px; background: var(--gradient); }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('wakasek.absensi') }}" class="sidebar-item active">
            <div class="sidebar-item-icon">📅</div>
            Rekap Absensi
        </a>
...
<main class="main">
    <div class="topbar">
        <div>
            <div class="topbar-title">Rekap Absensi</div>
            <div class="topbar-sub">Rekap kehadiran seluruh siswa PKL</div>
        </div>
    ...
    <div class="content">
        <div class="page-header">
            <div>
                <div class="page-header-title">Rekap Absensi</div>
                <div class="page-header-sub">Total {{ $totalSiswa }} siswa terpantau</div>
            </div>
            <span class="badge-count">{{ $totalSiswa }} Siswa</span>
        </div>
        ...
        <div class="stat-grid">
            <div class="stat-card">
                ...
                <div class="stat-card-num" style="color: #10b981;">{{ $totalHadir }}</div>
                <div class="stat-card-label">Total Hadir</div>
            </div>
            <div class="stat-card">
                <div class="stat-card-icon">📝</div>
                <div class="stat-card-num" style="color: var(--primary);">{{ $totalIzin }}</div>
                <div class="stat-card-label">Total Izin</div>
            </div>
            <div class="stat-card">
                <div class="stat-card-icon">🤒</div>
                <div class="stat-card-num" style="color: #f59e0b;">{{ $totalSakit }}</div>
                <div class="stat-card-label">Total Sakit</div>
            </div>
            <div class="stat-card">
                <div class="stat-card-icon">❌</div>
                <div class="stat-card-num" style="color: #ef4444;">{{ $totalAlpa }}</div>
                <div class="stat-card-label">Total Alpa</div>
            </div>
        </div>

        <!-- FILTER TABS -->
        <div class="filter-tabs">
            <button class="filter-tab active" data-filter="semua">Semua ({{ $totalSiswa }})</button>
            <button class="filter-tab" data-filter="hadir">✅ Ada Hadir</button>
            <button class="filter-tab" data-filter="alpa">❌ Ada Alpa</button>
        </div>
        ...
        <div class="search-bar">
            <div class="search-input-wrap">
                <span>🔍</span>
                <input type="text" class="search-input" id="searchInput" placeholder="Cari nama siswa...">
            </div>
        </div>
            ...
            <table class="absensi-table" id="absensiTable">
                ...
                <tbody>
                    ...
                    @php
                        ...
                        $filterVal = $alpa > 0 ? 'alpa' : ($hadir > 0 ? 'hadir' : 'semua');
                    @endphp
                        ...
                        <td>
                            <div class="siswa-wrap">
                                ...
                                <div>
                                    ...
                                    <div class="siswa-sub">{{ $siswa->email }}</div>
                                </div>
                            </div>
                        </td>
                        <td style="font-size: 13px; color: var(--text-muted);">
                            {{ $siswa->profilSiswa->kelas ?? '-' }}
                        </td>
                        <td>
                            <span class="absensi-pill pill-hadir">✅ {{ $hadir }}</span>
                        </td>
                        <td>
                            <span class="absensi-pill pill-izin">📝 {{ $izin }}</span>
                        </td>
                        <td>
                            <span class="absensi-pill pill-sakit">🤒 {{ $sakit }}</span>
                        </td>
                        <td>
                            <span class="absensi-pill pill-alpa">❌ {{ $alpa }}</span>
                        </td>
                        <td>
...
<script>
    ...
    document.querySelectorAll('.filter-tab').forEach(tab => {
        tab.addEventListener('click', function () {
            ...
            document.querySelectorAll('#absensiTable tbody tr').forEach(row => {
                row.style.display = (filter === 'semua' || row.dataset.filter === filter) ? '' : 'none';
            });
        });
    });
