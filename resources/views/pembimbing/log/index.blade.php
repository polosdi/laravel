{{-- ═══════════════════════════════════════════
     resources/views/pembimbing/log/index.blade.php
═══════════════════════════════════════════ --}}
...
.log-item:hover { background: rgba(102,126,234,.03); }
...
.log-icon.type-nilai     { background: rgba(234,179,8,.1);    color: #ca8a04; }
.log-icon.type-password  { background: rgba(239,68,68,.1);    color: #ef4444; }
.log-icon.type-absensi   { background: rgba(20,184,166,.1);   color: #0d9488; }
.log-icon.type-default   { background: rgba(102,126,234,.06); color: var(--text-muted); }
...
/* ── CARD OVERRIDE (match log siswa punya border-radius 18px) ── */
.card {
...
<div class="pg-head">
    <div class="pg-head-l">
        ...
        <h2>
            <div class="h-ic"><i class="fa-solid fa-clock-rotate-left"></i></div>
            Log Aktivitas
        </h2>
    </div>
</div>
...
<div class="card">
    ...
    <ul class="log-list">
        ...
        @php
            ...
            elseif (str_contains($act, 'jurnal')    || str_contains($act, 'Jurnal')
                 || str_contains($act, 'validasi')  || str_contains($act, 'Validasi'))          { $type = 'jurnal';   $ico = 'fa-book-open'; }
            elseif (str_contains($act, 'laporan')   || str_contains($act, 'Laporan'))           { $type = 'laporan';  $ico = 'fa-file-lines'; }
            elseif (str_contains($act, 'PKL')       || str_contains($act, 'pkl')
                 || str_contains($act, 'pengajuan') || str_contains($act, 'Pengajuan'))         { $type = 'pkl';      $ico = 'fa-clipboard-list'; }
            elseif (str_contains($act, 'nilai')     || str_contains($act, 'Nilai'))             { $type = 'nilai';    $ico = 'fa-star'; }
            elseif (str_contains($act, 'absensi')   || str_contains($act, 'Absensi'))           { $type = 'absensi';  $ico = 'fa-calendar-check'; }
