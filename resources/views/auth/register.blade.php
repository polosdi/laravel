...
<head>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page { position:relative;z-index:10;display:grid;grid-template-columns:1fr 460px;min-height:100vh; }

        /* LEFT — exact same as login */
        .left { padding:80px 48px 64px 80px;display:flex;flex-direction:column;justify-content:center; }
        .tagline { font-size:clamp(2rem,3vw,3.2rem);font-weight:800;color:var(--text);line-height:1.15;margin-bottom:22px;letter-spacing:-.02em;transition:color .4s; }
        .tagline .acc { background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text; }
        .ldesc { font-size:.92rem;color:var(--left-desc);line-height:1.78;max-width:380px;margin-bottom:44px;font-weight:400;transition:color .4s; }
        .feats { display:flex;flex-direction:column;gap:14px; }
        .feat { display:flex;align-items:center;gap:16px; }
        .feat-ico { width:42px;height:42px;border-radius:12px;background:var(--feat-bg);border:1px solid var(--feat-border);display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all .4s; }
        .feat-ico svg { width:18px;height:18px;color:#667eea; }
        .feat-txt { font-size:.855rem;color:var(--left-feat);font-weight:500;transition:color .4s; }

        /* RIGHT — exact same as login */
        .right { display:flex;align-items:center;justify-content:center;padding:70px 32px 32px;min-height:100vh; }

        /* CARD — exact same as login */
        .card { width:100%;max-width:380px;background:var(--card-bg);backdrop-filter:blur(28px) saturate(1.4);-webkit-backdrop-filter:blur(28px) saturate(1.4);border:1px solid var(--card-border);border-radius:20px;padding:24px 26px;box-shadow:var(--card-shadow);animation:cardIn .7s cubic-bezier(.34,1.3,.64,1) both;transition:background .4s,border-color .4s,box-shadow .4s; }
        ...
        /* RESPONSIVE — exact same as login */
        @media (max-width:960px) { .page{grid-template-columns:1fr} .left{display:none} .right{padding:80px 20px 32px;min-height:100vh} .topbar{padding:16px 20px} }
            ...
            <form method="POST" action="{{ route('register') }}" id="regForm">
                ...
                <div class="step-panel" id="p2">
                    ...
                    <div class="field">
                        ...
                        <div class="iw">
                            <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></span>
                            <input type="text" name="nama_lengkap" placeholder="Nama sesuai ijazah" value="{{ old('nama_lengkap') }}">
                        </div>
                    </div>
                    ...
                    <div class="field" style="margin-top:10px">
                        ...
                        <div class="iw">
                            ...
                            <textarea name="alamat" placeholder="Alamat lengkap tempat tinggal">{{ old('alamat') }}</textarea>
                        </div>
                    </div>
