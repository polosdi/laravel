...
<head>
    ...
    <title>Log Aktivitas – SIMPKL</title>
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('wakasek.log') }}" class="sidebar-item active">
...
<main class="main">
    <div class="topbar">
        <div>
            ...
            <div class="topbar-sub">Riwayat seluruh aktivitas pengguna sistem</div>
        </div>
    ...
    <div class="content">
        <div class="page-header">
            <div>
                ...
                <div class="page-header-title">Total {{ $daftarLog->count() }} aktivitas tercatat</div>
            </div>
            <span class="badge-count">{{ $daftarLog->count() }} Log</span>
        </div>
        ...
        <div class="search-bar" style="margin-bottom: 20px;">
            <div class="search-input-wrap">
                <span>🔍</span>
                <input type="text" class="search-input" id="searchInput" placeholder="Cari nama pengguna atau aktivitas...">
            </div>
        </div>

        <!-- TABLE -->
        <div class="card">
            <table class="absensi-table" id="logTable">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Pengguna</th>
                ...
                <tbody>
                    @forelse($daftarLog as $i => $log)
                    <tr>
                        <td style="color: var(--text-muted); font-size: 13px;">{{ $i + 1 }}</td>
                        <td>
                            <div class="siswa-wrap">
                                <div class="siswa-avatar">
                                    {{ strtoupper(substr($log->user->nama_depan ?? '?', 0, 1)) }}
                                </div>
                                <div>
                                    <div class="siswa-name">{{ $log->user->nama_depan ?? 'Sistem' }} {{ $log->user->nama_belakang ?? '' }}</div>
                                    <div class="siswa-sub">{{ $log->user->email ?? '-' }}</div>
                                </div>
                            </div>
                        </td>
                        <td>
                        ...
                        <td style="max-width: 300px;">{{ $log->aktivitas }}</td>
                    ...
                    <tr>
                        <td colspan="5">
                            <div class="empty-state">
                                ...
                                <div class="empty-state-text">Belum ada log aktivitas tercatat.</div>
                            </div>
                        </td>
                    </tr>
                    @endforelse
